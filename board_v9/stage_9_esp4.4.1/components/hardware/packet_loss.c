...
esp_err_t packet_loss_resend_request(uint8_t* resend_request, uint32_t size, bool way_to_send)
{
	...
	if (num_of_ranges > PACKET_RESEND_NUM_OF_RANGES_MAX)
    {
        ESP_LOGE(TAG_RESEND, "ERROR: NUMBER OF RANGES TO RESEND (%d) IS GREATER THAN MAX ALLOWED (%d)", num_of_ranges, PACKET_RESEND_NUM_OF_RANGES_MAX);
        init_resend_request_parameters_L();
        return ESP_FAIL;
    }
            ...
            if (manager_prepared_packet_request()==true)
            {
                ...
                else
                {
                    if (true==resend_packet_sn_any_method(sn_short_or_normal,VIA_BT,current_sn_to_send))
                    {
                        if (sn_short_or_normal == FULL_PACKET_SEND)
                        {
                            ets_printf("S%06X,RES,%llu[us]\r\n",current_sn_to_send,esp_timer_get_time());//ets_printf("SN 0x%06X res on %llu[us]\r\n",current_sn_to_send,esp_timer_get_time());
                        }
                        else
                        {
                            ets_printf("S%06X-S%06X,RES,%llu[us]\r\n",current_sn_to_send,current_sn_to_send+3,esp_timer_get_time());//ets_printf("SN 0x%06X - 0x%06X, res on %llu[us]\r\n",current_sn_to_send,current_sn_to_send+3,esp_timer_get_time());
                        }
                    }
