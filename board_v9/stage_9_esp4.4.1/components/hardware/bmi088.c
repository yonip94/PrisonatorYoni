...
/****************************************************************//**
 * @brief   configure the desired settings to the bmi sensor
 * 
 * @param   [IN] none
 * @return  none
 *******************************************************************/
void reconfigure_bmi_sensor(void)
{
    uint8_t buff_tx = 0x00;
    
    for (int i=0;i<6;i++)
    {
        buff_tx = ACC_PWR_CTRL_ACC_ON;
        spi_imu_write_reg(i, ACC_PWR_CTRL_ADDRESS, &buff_tx, sizeof(buff_tx));

        buff_tx = ACC_PWR_CONF_ACTIVE_MODE;
        spi_imu_write_reg(i, ACC_PWR_CONF_ADDRESS, &buff_tx, sizeof(buff_tx));

        buff_tx = ACC_CONF_VAL;
        spi_imu_write_reg(i, ACC_CONF_ADDRESS, &buff_tx, sizeof(buff_tx));
    }	
    //print_bmi_conf_err_regs();
}

/****************************************************************//**
 * @brief   print bmi important registers (err , pwr conf, pwr ctrl, conf registers)
 * 
 * @param   [IN] none
 * @return  none
 *******************************************************************/
void print_bmi_conf_err_regs(void)
{
    ...
    for (int i=0;i<6;i++)
    {
        printf("imu %d:\r\n",i);
        reg_data = 0xff;
        reg_data = acc_read_reg_L(i, 0x02);
        printf("err reg = 0x%02X\r\n",reg_data);
        reg_data = 0xff;
        reg_data = acc_read_reg_L(i, ACC_PWR_CONF_ADDRESS);
        printf("pwr conf reg = 0x%02X\r\n",reg_data);
        reg_data = 0xff;
        reg_data = acc_read_reg_L(i, ACC_PWR_CTRL_ADDRESS);
        printf("pwr ctrl reg = 0x%02X\r\n",reg_data);
        reg_data = 0xff;
        reg_data = acc_read_reg_L(i, ACC_CONF_ADDRESS);
        printf("conf reg = 0x%02X\r\n",reg_data);
    }	
}
...
esp_err_t BMI088_init(imu_all_t *imu_all)
{
    ...
    //uart_driver_install(UART_NUM_0, PACKET_NORM_SIZE, 0, 0, NULL, 0);
    ////////////////////////////////////////////////
...
esp_err_t BMI088_getData(imu_all_t *imu_all, uint64_t sample_time)
{
    ...
    for (uint8_t sample_id = 0; sample_id < IMU_NUM; sample_id++)
    {
        ...
        /***************************************************************/
        BUSY_WAIT_TIMEOUT_ERR( ((ACC_DATA_READY_MSK & acc_read_reg_L(cs, ACC_STATUS_ADDRESS)) != ACC_DATA_READY_MSK), IMU_TIMEOUT);
        ...
        /***************************************************************/
        BUSY_WAIT_TIMEOUT_ERR(  ((GYR_DATA_READY_MSK & gyr_read_reg_L(cs, GYR_INT_STAT1_ADDRESS)) != GYR_DATA_READY_MSK), IMU_TIMEOUT);
...
static esp_err_t init_imu_L(SPI_CS_T cs)
{
    ...
    /***********************************************************/
    //if (ACC_CHIP_ID_VAL!=acc_read_reg_L(cs_acc, ACC_CHIP_ID_ADDRESS))
    //{
    //    ets_printf("acc id read fail\r\n");
    //}

    /***********************************************************/
    // check ACC ID 
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((ACC_CHIP_ID_VAL != acc_read_reg_L(cs_acc, ACC_CHIP_ID_ADDRESS)), IMU_TIMEOUT);
    ...
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((ACC_PWR_CTRL_ACC_ON != acc_read_reg_L(cs_acc, ACC_PWR_CTRL_ADDRESS)), IMU_TIMEOUT);
    ...
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((ACC_PWR_CONF_ACTIVE_MODE != acc_read_reg_L(cs_acc, ACC_PWR_CONF_ADDRESS)), IMU_TIMEOUT);
    ...
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((ACC_RANGE_24G != acc_read_reg_L(cs_acc, ACC_RANGE_ADDRESS)), IMU_TIMEOUT);
    ...
    // wait till reg is updated
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((ACC_CONF_VAL != acc_read_reg_L(cs_acc, ACC_CONF_ADDRESS)), IMU_TIMEOUT);

    /***********************************************************/
    // check gyro ID 
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((GYR_CHIP_ID_VAL != gyr_read_reg_L(cs_gyr, GYR_CHIP_ID_ADDRESS)), IMU_TIMEOUT);
    ...
    // wait till reg is updated
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((GYR_RANGE_0 != gyr_read_reg_L(cs_gyr, GYR_RANGE_ADDRESS)), IMU_TIMEOUT);
    ...
    // wait till reg is updated
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((GYR_BW_1 != gyr_read_reg_L(cs_gyr, GYR_BW_ADDRESS)), IMU_TIMEOUT);
    ...
    // wait till reg is updated
    /***********************************************************/
    BUSY_WAIT_TIMEOUT_ERR((buff_tx != gyr_read_reg_L(cs_gyr, GYRO_INT_CTRL_ADDRESS)), IMU_TIMEOUT);
...
static uint8_t acc_read_reg_L(int cs, uint8_t addr)
{
    ...
    if (ESP_OK!=spi_imu_read_reg(cs, addr, buff_rx, sizeof(buff_rx)))
    {
        ets_printf("fail to read imu acc reg\r\n");
    }
    else
    {
        //ets_printf("acc addr 0x%02X, content = 0x%02X\r\n",addr,buff_rx[1]);
    }
    return buff_rx[1];
}
...
static uint8_t gyr_read_reg_L(int cs, uint8_t addr)
{
    ...
    if (ESP_OK!=spi_imu_read_reg(cs, addr, buff_rx, sizeof(buff_rx)))
    {
        ets_printf("fail to read imu gyr reg\r\n");
    }
    else
    {
        //ets_printf("gyr addr 0x%02X, content = 0x%02X\r\n",addr,buff_rx[0]);
    }
