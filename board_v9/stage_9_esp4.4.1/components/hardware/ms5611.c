...
MS5611_STATUS_T ms5611_write_command_L(uint8_t cmd)
{
    ...
    ret = i2c_master_write_slave(IOEXP_BARO_BAT_MMC_I2C_CHANNEL_PORT, MS5611_I2C_ADDR, data, 1, 2);
    if (ret != ESP_OK)
    {
...
static MS5611_STATUS_T ms5611_conversion_and_read_adc_L(uint8_t cmd, uint32_t *adc)
{
    ...
    ret = i2c_master_read_slave(IOEXP_BARO_BAT_MMC_I2C_CHANNEL_PORT, MS5611_I2C_ADDR, buffer, 3, 2);
    if (ret != ESP_OK)
    {
        return MS5611_STATUS_TRANSFER_ERR;
    }
...
MS5611_STATUS_T ms5611_read_eeprom_coeff_L(uint8_t command, uint16_t *coeff)
{
    ...
    ret = i2c_master_read_slave(IOEXP_BARO_BAT_MMC_I2C_CHANNEL_PORT, MS5611_I2C_ADDR, buffer, 2, 2);
    if (ret != ESP_OK)
    {
        return MS5611_STATUS_TRANSFER_ERR;
    }
