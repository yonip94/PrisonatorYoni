...
int inv_icm426xx_get_data_from_registers(struct inv_icm426xx * s)
{
	...
	status = inv_icm426xx_read_reg(s, MPUREG_TEMP_DATA0_UI, ICM_TEMP_DATA_SIZE, temperature);
	inv_icm426xx_format_data(s->endianess_data, temperature, (uint16_t *)&event.temperature);
	
	status |= inv_icm426xx_read_reg(s, MPUREG_ACCEL_DATA_X0_UI, ICM_ACCEL_DATA_SIZE, accel);
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_GYRO_DATA_X0_UI, ICM_GYRO_DATA_SIZE, gyro);
