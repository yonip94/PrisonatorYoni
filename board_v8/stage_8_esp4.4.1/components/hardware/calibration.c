...
esp_err_t calibration_after_powerup(void)
{
	...
	while (1)
    {
        if ((esp_timer_get_time()-timer_start)>=(POWER_OFF_BECAUSE_NO_COMMUNICATION_FOR_11_MIN))
        {
...
void copy_buff_to_mag_calib_uart(uint8_t * buff_to_cpy, uint16_t buff_size)
{
    ...
    if (buff_size==11)
    {
        memcpy( (cal_data_new+(buff_to_cpy[10]*BT_PACKET_CALIBRATION_SIZE) ) ,buff_to_cpy+0 , 10);
    ...
    {
        memcpy( (cal_data_new+(buff_to_cpy[11]*BT_PACKET_CALIBRATION_SIZE) + (buff_to_cpy[12]*10) ) ,buff_to_cpy+1 , 10);
...
void copy_buff_to_mag_calib_bt(uint8_t * buff_to_cpy, uint16_t buff_size)
{
    ...
    if (buff_size==301)
    {
        memcpy( (cal_data_new+(buff_to_cpy[300]*BT_PACKET_CALIBRATION_SIZE) ) ,buff_to_cpy+0 , 300);
    ...
    {
        memcpy( (cal_data_new+(buff_to_cpy[301]*BT_PACKET_CALIBRATION_SIZE) + (buff_to_cpy[302]*300) ) ,buff_to_cpy+1 , 300);
...
void set_mag_calib_done_flag(bool way_to_send)
{
    ...
    memcpy((cal_data_new+BT_PACKET_CALIBRATION_SIZE),(cal_data_old+BT_PACKET_CALIBRATION_SIZE),(BT_PACKET_CALIBRATION_TOTAL_SIZE-BT_PACKET_CALIBRATION_SIZE));
}
...
void erase_calibration_data(bool way_to_send)
{
    ...
    {
        cal_data_new[i*BT_PACKET_CALIBRATION_SIZE]=0x01;
        cal_data_new[(i*BT_PACKET_CALIBRATION_SIZE)+1]=i;
    }
...
static esp_err_t send_calibration_data_L(bool way_to_send)
{
    ...
    for (packet_num = 0; packet_num<BT_PACKET_CALIBRATION_NUM; packet_num++)
    {
        ...
        {
            /* SW version */
            memcpy(cal_packet + BT_PACKET_OFFET_CAL_SW_VERSION, app_desc->version, sizeof(app_desc->version)); 
            
            /* HW version */
            memcpy(cal_packet + BT_PACKET_OFFET_CAL_HW_VERSION, &hw_version, sizeof(hw_version)); 
        }
        ...
        {
            if (ESP_OK!=bt_send_data(cal_packet, BT_PACKET_CALIBRATION_SIZE))
            {
