...
static uint8_t tmp_value=0;//remove after debug
...
static uint8_t done_flag = 0;//todo del after debug
static uint8_t uart_is_interrupt_on_going = 0;
//static uint32_t current_sn_test =0; //uncomment for debug uart send data
 ...
 * @return  none
 *******************************************************************/
void uart_bt_packet_send_buff(uint8_t* buff)
{
    //byte read
    ...
    stream_size = encode_L(buff,uart_data_send,BT_PACKET_NORM_SIZE);
}
 ...
 * @return  none
 *******************************************************************/
void uart_bt_packet_resend_buff(uint8_t* buff,uint8_t* encoded_to_resend_buff)
{
...
void uart_send_calibration_data(uint8_t* buffer)
{
        ...
        for(uint16_t ind_y=0;ind_y<(BT_PACKET_CALIBRATION_SIZE/25);ind_y++)
        {
...
static void uart_get_task_L(void *arg)
{
    ...
    for (;;)
    {
        ...
        /***********************************************************/
        // only if packet arrived and resend process finished or not start
        // checking the packet, otherwise the resend request will be lost
        /***********************************************************/
        if((uart_receive_flag==USER_PACKET_ARRIVED)&&(uart_resend_flag==RESEND_PACKET_FINISHED))
        {
            /*******************************************************/
            // if at least 1 packet byte is invalid - the whole packet is ignored
            /*******************************************************/
            memcpy(uart_rx_buf,rxbuf,UART_RECEIVE_HEADER_SIZE);
            memset(rxbuf,0x00,UART_RECEIVE_HEADER_SIZE);

            /*******************************************************/
            //if resend request packet arrived and the last resend process
            /*******************************************************/
            ...
            else
            {
                ...
                { 
                    ...
                    case (CALIBRATION_APP_SEND_TO_BOARD_PARTN_SIZE):
                    {
                        ...
                        if(uart_rx_buf[BT_PACKET_OFFET_CAL_TYPE] == BT_PACKET_TYPE_VAL_CAL)
                        {
                            if ( ((uart_rx_buf[11]>=0) && (uart_rx_buf[11]<=8)) &&
                                 ((uart_rx_buf[12]>=0) && (uart_rx_buf[12]<=89))  )
                            {
                    ...
                    case (1):
                    {
                        ...
                        //else if (uart_rx_buf[BT_PACKET_OFFET_TYPE] == '1')//uncomment - for debug uart ka
                        //{
                        ...
                        //else if (uart_rx_buf[BT_PACKET_OFFET_TYPE] == '2')//uncomment - for debug uart ack initial calib
                        //{
                        //    uart_calibration_status_flag=true;
                        //    ets_printf("ACK ARRIVED\r\n");
                        //}

                        //else if ((uart_rx_buf[BT_PACKET_OFFET_TYPE] == '3'))//uncomment - for debug uart resend data
                        //{
                        //    current_sn_test=manager_send_packet_sn();
